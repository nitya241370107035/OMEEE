...
def run_aoi_ingestion_pipeline(
    ...
    Executes Entry Point A:
      ...
      3. For each scene:
         - Streams 5 bands (RGB + NIR + SWIR) & reprojects to EPSG:4326
         ...
         - Upserts scenes & tiles into Postgres
    """
    t0 = time.time()
    logger.info("=" * 75)
    logger.info(f"STARTING ENTRY POINT A INGESTION: region='{region_id}' ({date_from} to {date_to})")
    logger.info(f"Time Buckets: {num_time_buckets} | Max Cloud: {max_cloud_cover}%")
    logger.info("=" * 75)
    ...
    for scene in scenes:
        logger.info(f"\n--- Processing Scene: {scene.scene_id} ({scene.time_bucket or 'single'}) ---")

        # Phase 1.2: 5-Band Canvas Assembly
        canvas: CanvasData = assemble_canvas_from_stac(
            scene_meta=scene,
            aoi_bbox=aoi.bbox,
            buffer_pct=0.05
        )
        ...
        manifest_path = save_tiles_and_manifest(
            ...
            source_type="aoi_search",
            extra_properties={"time_bucket": scene.time_bucket, "cloud_cover": scene.cloud_cover}
        )
        ...
        if populate_db:
            try:
                upsert_scene(
                    ...
                    acquisition_date=scene.acquisition_date,
                    crs=scene.crs,
                    footprint_geom=aoi.polygon,
                    raw_file_path=scene.assets.red,
                    provenance={"time_bucket": scene.time_bucket, "sensor": scene.sensor}
                )
                upsert_tiles(
                    tiles=tiles,
                    scene_id=scene.scene_id,
