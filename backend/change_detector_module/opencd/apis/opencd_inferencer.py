...
class OpenCDInferencer(MMSegInferencer):
    ...
    def __init__(self,
        ...
        if isinstance(self.model.dataset_meta, dict):
            meta_classes = self.model.dataset_meta.get('classes')
            meta_palette = self.model.dataset_meta.get('palette')
        elif self.model.dataset_meta is not None:
            meta_classes = getattr(self.model.dataset_meta, 'classes', None)
            meta_palette = getattr(self.model.dataset_meta, 'palette', None)
        else:
            meta_classes = None
            meta_palette = None

        classes = classes if classes else meta_classes
        palette = palette if palette else meta_palette
